...
test_that("calculates correct value for square", {
  ...
  # error radius of 500 gives a 500x500 m square
  min_eoo <- eooMin(points, defaultRadius=500)
...
test_that("returns an sf with a polygon geometry", {
  ...
  min_eoo_sf <- eooMin(points, defaultRadius=100, returnV="SF")
  
  expect_s3_class(min_eoo_sf$geometry[[1]], "POLYGON")
})
